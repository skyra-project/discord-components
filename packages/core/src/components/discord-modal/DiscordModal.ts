...
export class DiscordModal extends LitElement implements LightTheme {
	...
	protected handleFormSubmit(event: SubmitEvent) {
		...
		for (const index of slotedItems!) {
			...
			const textarea = shadowRootSlot?.querySelectorAll('textarea').item(0);
			const input = shadowRootSlot?.querySelectorAll('input').item(0);
			if (textarea) {
				...
				if (textarea.value.length < Number(textarea.attributes.getNamedItem('minlength')?.value)) return;
			} else if (input?.attributes.getNamedItem('required') && !input.value) {
